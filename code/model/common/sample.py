...
class NormalAroundPoint(Sampler):
    def get_points(self,pc_input):
        ...
        dists = [torch.distributions.MultivariateNormal(torch.zeros(dim).cuda(),
                                                      torch.diag(torch.Tensor([sigma**2]*dim)).cuda())
...
class NormalAroundPointWithUniform(NormalAroundPoint):
    def get_points(self,pc_input):
        ...
        return torch.cat([normal_around_points,
                          torch.empty(pc_input.shape[0] - num_points,pc_input.shape[1]).uniform_(-1.1,1.1).cuda()],dim=0)
