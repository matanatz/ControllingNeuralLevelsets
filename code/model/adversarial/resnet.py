...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        #nn.init.normal_(self.linear.weight,1./(512 * block.expansion),0.00001)
    ...
    def __init__(self):
        ...
        for m in self.model.modules():
            if isinstance(m, nn.Conv2d):
                ...
                torch.nn.init.normal_(m.weight,0,np.sqrt(2. / n))
                #m.weight.data.normal_(0, )
                #m.bias.data.zero_()
        self.register_forward_hook(ResNet18.hook)
